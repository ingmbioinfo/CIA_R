...
The `signature_score` function calculates the signature scores across all given signatures and cells, utilizing parallel processing to enhance efficiency. 
...
```{r wrappedrun}
MTG <- signature_based_classification(
  data = MTG,
  signatures_input = gmt,
  n_cpus = 2,
...
The similarity threshold is a critical parameter in the classification process, utilized within the `signature_based_classification` function. 
...
TODO: we never used so far `classification_metrics()`...
