...
download.file(url = "https://sea-ad-single-cell-profiling.s3.us-west-2.amazonaws.com/MTG/RNAseq/Reference_MTG_RNAseq_all-nuclei.2022-06-07.rds", 
...
```{r compsignatures}
scores <- compute_signature_scores(data = MTG, 
                                   seurat_assay = "RNA", 
                                   matrix = "data", 
                                   geneset = gmt$Astro)
head(scores)
...
```{r compsig2}
scores <- compute_signature_scores(data = MTG_SE, 
                                   matrix = "logcounts", 
                                   geneset = gmt$Astro)
head(scores)
```
...
```{r sigscore2}
MTG_SE <- signature_score(data = MTG_SE, 
                          signatures_input = gmt, 
                          return_score = FALSE, 
                          score_mode = "scaled",
                          n_cpus = 10)
...
```{r plotastrovips, fig.width=16, fig.height=6}
DimPlot(MTG, raster = FALSE)|
...
```{r wrappedrun}
MTG <- signature_based_classification(data = MTG, 
                                      signatures_input = gmt,
                                      n_cpus = 10,
...
```{r plotgrouped, fig.width=16, fig.height=8}
DimPlot(MTG, raster = FALSE) | 
...
```{r plotnoastro, fig.width=16, fig.height=8}
DimPlot(MTG, raster = FALSE) | 
